...
cli.responders.help = () =>{
    let commands = {
        ...
        'list logs': 'Sho a list of all the log files available to be read (compressed only)',
...
// List logs
cli.responders.listLogs = () =>{
    let ls = childProcess.spawn('ls',['//.logs//'],{'shell':true});
...
// More logs info
cli.responders.moreLogsInfo = (str) =>{
    // Get the log filename from the string
    let arr = str.split('--');
    ...
    if(str){
        ...
        uniqueInputs.some(input => {
            if(str.toLowerCase().indexOf(input) > -1){
                matchFound = true;
                // Emit an event matching the unique input, include the full string given
                e.emit(input,str);
                return true;
            }
        });
