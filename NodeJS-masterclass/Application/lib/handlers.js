...
let PerformanceObserver = require('perf_hooks').PerformanceObserver;
      ...
      if(!err && userData){
        ...
        if(hashedPassword == userData.hashedPassword){
          ...
          _data.create('tokens',tokenId,tokenObject,function(err){
            ...
            // Create an obsever for the measures. The callback will print the name and duration of the measurement
            let obs = new PerformanceObserver((list, observer) => {
              list.getEntriesByType('measure').forEach(measurement =>{
                debug('\x1b[33m%s\x1b[0m',measurement.name+' took '+measurement.duration+' ms');
              });              
              observer.disconnect();
            });
